...
var viewPortHeight=$(window).height();var scrollFromTop=$(window).scrollTop();var scrollFromBottom=(parseInt(scrollFromTop)+parseInt(viewPortHeight));function recalcVars(){viewPortHeight=$(window).height();scrollFromTop=$(window).scrollTop();scrollFromBottom=(parseInt(scrollFromTop)+parseInt(viewPortHeight));for(var i=0;i<elementsToTrack.length;i++){checkInViewport(scrollFromBottom,elementsToTrack[i]);}
if(useMiniMap){$('.topmarker').css('height',scrollFromTop+'px');$('.bottommarker').css('top',(scrollFromBottom)+'px');}}
$(window).scroll(function(e){recalcVars();});$(window).resize(function(e){recalcVars();});function checkInViewport(scrollBottom,domElement){var elementPos=$(domElement).offset().top;if((parseInt(scrollBottom)+parseInt(pixelOffset))>elementPos){$(domElement).addClass(inClassName).removeClass(outClassName);}else{$(domElement).removeClass(inClassName).addClass(outClassName);}}
if(useMiniMap){$(document).ready(function(){$('body').clone().appendTo('body').addClass('mini');$('body.mini').append("<span class='topmarker'></span>");$('.topmarker').css('position','absolute').css('top','0px').css('height',scrollFromTop+'px').css('background-color','rgba(0,0,0,0.6)').css('display','block').css('width','100%');$('body.mini').append("<span class='bottommarker'></span>");$('.bottommarker').css('position','absolute').css('top',(scrollFromBottom)+'px').css('height','40000px').css('background-color','rgba(0,0,0,0.6)').css('display','block').css('width','100%');});}